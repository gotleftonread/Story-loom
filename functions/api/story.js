...
export async function onRequestPost(context) {
  ...
  if (!apiKey) {
    return jsonResponse(
      { error: { message: "Server is missing GEMINI_API_KEY. Add it in Cloudflare Pages > Settings > Environment variables." } },
      500
    );
  }
  ...
  try {
    geminiRes = await fetch(
      `https://generativelanguage.googleapis.com/v1beta/models/gemini-3.0-flash-live:generateContent?key=${apiKey}`,
      {
        method: "POST",
